...
// The portrait offered to an administrator in the Media section, generated by
// tools/make-headshot.ps1. Absolute rather than relative on purpose: the same
// URL has to work inside an exported signature sitting in somebody's mail
// client, where a relative path resolves against nothing.
const ADMIN_PORTRAIT_URL = 'https://signvel.com/admin-portrait.jpg';
...
function renderMedia() {
  ...
  // Admin-only: one real head-and-shoulders portrait, to see a photo layout
  // against something other than the stock face. Hidden from everyone else for
  // the same reason that picker was removed — a company asset is not something
  // to invite a customer to ship as their own. isAdmin() is presentation only,
  // and nothing here needs protecting: the file is a public image either way.
  if (isAdmin()) {
    const inUse = S.headshotUrl === ADMIN_PORTRAIT_URL;
    h += `<div class="field-row"><label class="field-label">Admin portrait<span class="field-hint">Only an admin sees this row.</span></label>
      <div class="add-chips">
        <button class="chip${inUse ? ' active' : ' accent'}" data-action="adminPortrait">${inUse ? 'In use' : 'Try it'}</button>
        <button class="chip" data-action="adminPortraitClear">Back to sample</button>
      </div></div>`;
  }
    ...
    if (togAction) {
      ...
      switch(action) {
        ...
        case 'sampleHeadshot':
          ...
          break;
        // Guarded here as well as in the panel. The row is only drawn for an
        // admin, but a data-action is a string in a page anyone can read, and
        // a control that works for whoever types it is not a control.
        case 'adminPortrait':
          if (!isAdmin()) break;
          S.headshotUrl = ADMIN_PORTRAIT_URL;
          S.headshotName = 'Admin portrait';
          S.uploadError = ''; S.storageError = '';
          break;
        case 'adminPortraitClear':
          if (!isAdmin()) break;
...
function adoptCloudState(row) {
  ...
  // platform existed has a social list that predates it.
